...
const Problem = () => {
  ...
  useEffect(() => {
    ...
    const fetchTestCases = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/problems/${id}/testcase`, {
          headers: {
            Authorization: `${token}`,
          },
        });
    ...
    const fetchSampleTestCases = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/problems/${id}/sampletestcase`, {
          headers: {
            Authorization: `${token}`,
          },
        });
  ...
  const handleRunCode = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/run', {
        code: userCode,
        input: stestCaseInput,// Send custom input
        lang: language,
        headers: {
  ...
  const handleRunCustomInput = async () => {
    try {
      const res = await axios.post('http://localhost:5000/run', {
        code: userCode,
        input: customInput, // Send custom input
        lang: language,
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const handleSubmitCode = async () => {
    try {
      ...
      if (comment.includes('Compilation Failed')) {
